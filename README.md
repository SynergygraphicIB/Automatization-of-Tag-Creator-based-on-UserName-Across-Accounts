...
This is an open-source solution to deploy **AutoTagging** using `CloudTrail` and channel the event invoke through `Cloudwatch Events` and `EventBrigdge` across accounts to a `Lambda function` to `tag resources` at the moment of creation with the arn of who created, the username ID, and the time of creation. 
