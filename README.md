...
**AutoTaggingExecuteLambda** - Role we create in every linked account whose policy only has limited permissions to do the tagging of newly deployed resources. This is the role that  ***AutoTaggingMasterLambda*** from *ReceiverAccount* assumes to make possible the recollection of creation events across accounts.
...
At the Ft. Lauderdale School of *Receiver Account* we will deploy a resource (John´s entering the school) in us-east-1, lets say a `Vpc` in the AWS environment, hence a creation event is generated. 
